...
async def get_week(auth_session: AuthSession, _date: str, folder="./cache/", rewrite: bool = False, verbose: bool = False) -> Week:
    ...
    week = await auth_session.fetch_week(_date, verbose=verbose)
...
async def get_weeks(auth_session: AuthSession, start: date, end: date, folder="./cache/", rewrite: bool = False, verbose: bool = False) -> list[Week]:
    current = start - timedelta(days=start.weekday())  # Monday
    weeks = []
    while current <= end:
        if verbose:
            print("getting week", current)
        weeks.append(await get_week(auth_session=auth_session, _date=str(current), folder=folder, rewrite=rewrite, verbose=verbose))
